...
int main(int argc, char* argv[])
{
    ...
    size_t ntsteps /*= (size_t) std::atoi(argv[1])*/, // 10000, //00,
           printvl /*= (size_t) std::atoi(argv[2])*/; // 1000;
    double dt /*= std::atof(argv[3])*/, //1e-5,
           //f  [>= std::atof(argv[4])<], //1.,
           //k  [>= std::atof(argv[5])<], //1.,
           //g  [>= std::atof(argv[6])<], //1.,
           L = 10. /*= std::atof(argv[7])*/; //10.;
           //g__ov__tau  = std::atof(argv[8]); //10.;
    ...
    while(std::getline(inputfile, line))
    {
        ...
        else if (instruction.at(0) == "bond")
        {
            ...
            if (instruction.at(iopt) == "harmonic")
            {
                ...
                bs.push_back(new BondHarmonic);
                auto ibond_ptr = static_cast<BondHarmonic*>(bs.back());
                ibond_ptr->id   = id;
    ...
    // for displaying time-stepping progress
    //boost::progress_display* show_progress_ptr = nullptr;
    //if (true) show_progress_ptr = new boost::progress_display(ntsteps);
...
//#pragma omp parallel for
        ...
        {
            auto iBond = static_cast<BondHarmonic*>(bs.at(ibond));
    ...
    //std::cout << "nbonds = " << bs.size() << std::endl;
    //for (int ibond = (int(nbonds)-1); ibond >= 0; ibond--)
    //{
        //auto iBond = static_cast<BondHarmonic*>(bs.at(ibond));
        //uint iatom = iBond->iatom;
        //uint jatom = iBond->jatom;
        ...
        //if (
                //((vs.at(iatom)->xyz[2] > threshold) && (vs.at(jatom)->xyz[2] < threshold)) ||
                //((vs.at(jatom)->xyz[2] > threshold) && (vs.at(iatom)->xyz[2] < threshold))
           //)
        //{
            ////std::cout << "nbonds = " << bs.size() << ", ibond = " << ibond << std::endl;
            //bs.erase(bs.begin() + ibond);
        //}
    //}
    ...
    //// integrate
    //for (uint tstep = ntsteps; tstep < 4*ntsteps; ++tstep)
    //{
        //// calculate center of mass
        //double x_cm = 0., y_cm = 0., z_cm = 0.;
        //for (size_t iatom = 0; iatom < natoms; ++iatom)
...
////#pragma omp parallel for reduction(+:vs)
        //for (size_t ibond = 0; ibond < nbonds; ++ibond)
        //{
            //auto iBond = static_cast<BondHarmonic*>(bs.at(ibond));
            //uint iatom = iBond->iatom;
            ...
            //iBond->r0 += g__ov__tau*(iBond->r - iBond->r0)*dt;
            ...
            //{
                //if (!vs.at(iatom)->frozen)
                //{
                    //vs.at(iatom)->vxyz.at(icoord) -= (tau*deltaR + iBond->addforce)*rp.at(icoord);
                //}
                //if (!vs.at(jatom)->frozen)
                //{
                    //vs.at(jatom)->vxyz.at(icoord) += (tau*deltaR + iBond->addforce)*rp.at(icoord);
                //}
            //}

            ////// add additional constant force
            ////for (uint icoord = 0; icoord < 3; ++icoord)
            ////{
                ////vs.at(iatom)->vxyz.at(icoord) -= zeta*rp.at(icoord);
                ////vs.at(jatom)->vxyz.at(icoord) += zeta*rp.at(icoord);
            ////}
        //}
